...
def format_data(data, parts, x_HEAD, y_HEAD, z_HEAD, frames, timestamps):
    ...
    ordered_df = pd.DataFrame(columns=mobi_motion_tracking_ordered_columns)
    ...
    for idx, column in enumerate(zed_ordered_columns):
        ordered_df[mobi_motion_tracking_ordered_columns[idx]] = data_df[column]

    # rename the headers of ordered_df to be mobi_motion_tracking_ordered_columns
    # Append corresponding frames as first column of ordered_df
    
    ordered_df.insert(0, "frame", frames)
    # ordered_df.insert(0, 'timestamp', timestamps)
